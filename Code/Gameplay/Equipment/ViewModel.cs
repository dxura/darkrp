...
public partial class ViewModel : Component, IEquipment
{
	...
	/// </summary>
	private Player Owner => Equipment.IsValid() ? Equipment.Owner : null;
	...
	{
		var moveVel = Owner.Velocity;
		var moveLen = moveVel.Length;

		var wishMove = Owner.WishMove.Normal * 1f;
		if ( Equipment?.Tags.Has( "aiming" ) ?? false )
		{
			wishMove = 0;
		}

		if ( Owner.IsSlowWalking || Owner.IsCrouching )
		{
			moveLen *= 0.5f;
		}
	...
	private void ApplyAnimationParameters()
	{
		...
		// Weapon state
		ModelRenderer.Set( "b_empty",
			!Equipment.Components.Get<AmmoComponent>( FindMode.EnabledInSelfAndDescendants )?.HasAmmo ?? false );
	}
	...
	[Property]
	[ShowIf( nameof(IsThrowable), true )]
	[Group( "Configuration" )]
	public ThrowableTypeEnum ThrowableType { get; set; }
	...
	protected override void OnUpdate()
	{
		...
		if ( !Owner.IsValid() || !Owner.IsValid() )
		{
			return;
		}
