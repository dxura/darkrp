	...
	IGameEventHandler<EquipmentHolsteredEvent>
{
	...
	[EquipmentResourceProperty]
	public Curve BaseDamageFalloff { get; set; } = new(new List<Curve.Frame>() { new(0, 1), new(1, 0) });
	...
	[Property]
	[Group( "Effects" )]
	public ParticleSystem PrimaryTracer { get; set; } =
		ParticleSystem.Load( "particles/gameplay/guns/trail/trail_smoke.vpcf" );
	...
	[Property]
	[Group( "Effects" )]
	public ParticleSystem SecondaryTracer { get; set; } =
		ParticleSystem.Load( "particles/gameplay/guns/trail/rico_trail_smoke.vpcf" );
	...
	[Property]
	[Group( "Ricochet" )]
	protected float RicochetMaxHits { get; set; } = 2f;
	...
	[Property]
	[Group( "Ricochet" )]
	public float MaxRicochetAngle { get; set; } = 45f;
	...
	[Property]
	[Group( "Fire Modes" )]
	public List<FireMode> SupportedFireModes { get; set; } = new() { FireMode.Automatic };
	...
	protected void DoShootEffects()
	{
		...
		if ( ShootSound is not null )
		{
			...
			{
				snd.ListenLocal = !Equipment.Owner?.IsProxy ?? false;
	...
	private LegacyParticleSystem CreateParticleSystem( ParticleSystem particleSystem, Vector3 pos, Rotation rot,
		float decay = 5f )
	{
	...
	private void CreateBloodEffects( Vector3 pos, Vector3 normal, Vector3 direction )
	{
		...
		if ( tr.Hit )
		{
			var material = RandomExtensions.FromList( Game.Random,
				GlobalGameNamespace.GetGlobal<PlayerGlobals>().BloodDecalMaterials );
			CreateDecal( material, tr.HitPosition - tr.Direction * 2, tr.Normal, Game.Random.Float( 0, 360 ),
				Game.Random.Int( 32, 96 ), 10f, 30f );
		}
	}

	private DecalRenderer CreateDecal( Material material, Vector3 pos, Vector3 normal, float rotation, float size,
		float depth, float destroyTime = 3f )
	{
		...
		decalRenderer.Size = new Vector3( size, size, depth );
	...
	private void CreateImpactEffects( Surface surface, Vector3 pos, Vector3 normal )
	{
		...
		{
			var decal = RandomExtensions.FromList( Game.Random, decalResource.Decals );

			CreateDecal( decal.Material, pos, normal, decal.Rotation.GetValue(), decal.Width.GetValue() / 1.5f,
				decal.Depth.GetValue(), 30f );
		}
	...
	public void Shoot()
	{
		...
		for ( var i = 0; i < BulletCount; i++ )
		{
			...
			foreach ( var tr in GetShootTrace() )
			{
				...
				if ( tr.GameObject?.Root.Components.Get<Player>( FindMode.EnabledInSelfAndDescendants ) is
				    { } player )
				{
				...
				if ( count > 0 )
				{
					damageFlags |= DamageFlags.WallBang;
				}
				...
				{
					InflictDamage( tr.GameObject, damage, tr.EndPosition, tr.Direction, tr.GetHitboxTags(),
						damageFlags );
				}
	...
	[Broadcast]
	private void InflictDamage( GameObject target, float damage, Vector3 pos, Vector3 dir, HitboxTags hitbox,
		DamageFlags flags )
	{
		if ( target.IsValid() )
		{
	...
	protected virtual IEnumerable<SceneTraceResult> GetShootTrace()
	{
		...
		forward += (Vector3.Random + Vector3.Random + Vector3.Random + Vector3.Random) *
		           (BulletSpread + Equipment.Owner.Spread) * 0.25f;
